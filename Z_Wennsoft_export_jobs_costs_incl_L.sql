--WITH Summed AS (
--    SELECT Job_Number,Phase_Code,Tran_Date_Text,Check_Number, SUM(Tran_Amount) AS Sum_Amount
--    FROM JC_TRANSACTION_HISTORY_MC
--	--FROM dbo.WO_COST_HISTORY_MC
--    --WHERE Pay_Type IN ('R', 'BASIC LIFE', 'LTD ER')
--	WHERE Check_Number LIKE 'E%'
--    GROUP BY Tran_Date_Text,Job_Number,Phase_Code,Check_Number
...
--	LEFT OUTER JOIN
--	Summed AS S
--		ON j1.Job_Number = S.Job_Number
--	LEFT OUTER JOIN
--	(SELECT Job_Number, Phase_Code, Cost_Type, Original_Est_Cost, Original_Est_Hours, Original_Est_Quantity
...
FROM
  ...
  FROM dbo.JC_PHASE_MASTER_MC WITH (NOLOCK)
  WHERE (Phase_Code = '0001')) AS j2 
    ON j1.Job_Number = j2.Job_Number 
  LEFT OUTER JOIN
  (SELECT LTRIM(RTRIM(Job_Number)) AS Job_Number, Cost_Type, Tran_Type_Code, Tran_Date_Text, Invoice_Number, Invoice_Date, Tran_Amount, PR_Hours1, PR_Hours2, 
    PR_Hours3, PR_Pay_Amount1, PR_Pay_Amount2, PR_Pay_Amount3, Quantity AS Qty, Tran_Amount AS Amt
  FROM dbo.JC_TRANSACTION_HISTORY_MC WITH (NOLOCK)
  WHERE (Company_Code = 'NA2') AND (Cost_Type = 'L')) AS Trx ON j1.Job_Number = Trx.Job_Number AND j2.Cost_Type = Trx.Cost_Type
  --LEFT OUTER JOIN
  --(SELECT LTRIM(RTRIM(Job_Number)) AS Job_Number, Cost_Type, Tran_Type_Code, Tran_Date_Text, Invoice_Number, Invoice_Date, Tran_Amount, PR_Hours1, PR_Hours2, 
  --  PR_Hours3, PR_Pay_Amount1, PR_Pay_Amount2, PR_Pay_Amount3, Quantity AS Qty, Tran_Amount AS Amt
  --FROM dbo.JC_TRANSACTION_HISTORY_MC WITH (NOLOCK)
  --WHERE (Company_Code = 'NA2') AND (Cost_Type = 'L')) AS L ON j1.Job_Number = L.Job_Number AND j2.Cost_Type = L.Cost_Type
WHERE
  (Trx.Tran_Type_Code <> '')
  --AND (j2.Cost_Type = 'L')
ORDER BY j1.Job_Number DESC
